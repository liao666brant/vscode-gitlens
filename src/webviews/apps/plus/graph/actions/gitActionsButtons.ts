	...
	private renderWipTooltip() {
		...
		${this.hasWorkingChanges
			? html`
					...
					工作更改
					<br />
					${stats!.added ? html`${stats!.added} 个文件 已添加<br />` : nothing}
					${stats!.modified ? html`${stats!.modified} 个文件 已修改<br />` : nothing}
					${stats!.deleted ? html`${stats!.deleted} 个文件 已删除<br />` : nothing}
				`
			: html`
					<hr />
					无更改
		...
		return html`
			<gl-tooltip placement="bottom">
				...
				>
					...
					<span class="publish-button__text">发布分支</span>
				</a>
				<span slot="content">
					发布（推送）${this.branchName ? html`<strong>${this.branchName}</strong>` : '此分支'}到远程仓库
				</span>
			</gl-tooltip>
		`;
	}
