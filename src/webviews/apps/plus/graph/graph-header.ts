						...
						>
							...
							<span slot="tooltip">
								${this._modifiers.altKey
									? html`跳转到引用...`
									: html`跳转到 HEAD<br />[${getAltKeySymbol()}] 跳转到引用...`}
							</span>
						</gl-button>
						<gl-button
							appearance="toolbar"
								...
								${when(
									...
									() => html`
										...
										<a href=${createCommandLink('gitlens.agents.installClaudeHook')}
											>安装 Claude Code Hooks</a
										>
										以便从 GitLens 查看和管理你的并行智能体工作。
									`,
								)}
							</div>
						</gl-popover>
					...
					() => html`
						<gl-popover class="hooks-tooltip" placement="bottom" trigger="click focus hover">
							...
							<div class="hooks-tooltip__content" slot="content">
								<strong>安装 Claude Code Hooks</strong><br />
								配置 Claude 向 GitLens 发送状态更新，以便你查看和管理并行智能体工作。
								<br /><br />
								<a href=${createCommandLink('gitlens.agents.installClaudeHook')}>安装</a>
								&middot;
								<a href=${createCommandLink('gitlens.agents.uninstallClaudeHook')}>卸载</a>
								&middot;
								<a
			...
			<gl-popover class="graph-walkthrough-tooltip" placement="bottom" trigger="hover focus" ?open=${highlighted}>
				...
				<div class="graph-walkthrough-tooltip__content" slot="content">
					<span class="graph-walkthrough-tooltip__title">
						<strong>体验全新提交图</strong>
						<span class="preview-badge">预览</span>
					</span>
					你的开发与智能体工作流在此交汇。超越历史可视化，管理、执行和并行化你的整个 Git 工作流。
					<div class="graph-walkthrough-tooltip__actions">
						<gl-button @click=${this.onGraphWalkthroughBannerButtonClick}>查看新功能</gl-button>
						<a href="#" @click=${this.onGraphWalkthroughBannerDismiss}>关闭</a>
					</div>
				</div>
			</gl-popover>
		`;
			...
			<div class="titlebar__row titlebar__row--search ${rowClass}">
				<div class="titlebar__group">
					...
					<span class="button-group">
						...
						<gl-button
							appearance="toolbar"
							tooltip=${config?.minimap && this.minimapVisible ? '隐藏小地图' : '显示小地图'}
							aria-label=${config?.minimap && this.minimapVisible ? '隐藏小地图' : '显示小地图'}
						...
						${(() => {
							...
							const baseLabel = this.detailsVisible ? '隐藏详情面板' : '显示详情面板';
							const altLabel = `在${altLocation === 'bottom' ? '底部' : '右侧'}显示详情面板`;
