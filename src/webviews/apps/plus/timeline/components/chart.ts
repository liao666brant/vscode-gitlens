...
export class GlTimelineChart extends GlElement {
	...
	private _renderRail(): unknown {
		...
		return html`<aside class="rail" aria-label="作者">${items} ${y2Axis}</aside>`;
	}
				...
				${this._zoomed
					? html`<gl-button
						...
						>
							...
							<span slot="tooltip">缩小<br />${getAltKeySymbol()} 重置缩放</span>
						</gl-button>`
					: nothing}
				<gl-button appearance="toolbar" @click=${() => this._zoomBy(0.5)} tooltip="放大" aria-label="放大">
	...
	private get _a11yWrapperLabel(): string {
		...
		if (count === 0) return '可视化历史时间线';

		const noun = count === 1 ? '次提交' : '次提交';
		return `可视化历史时间线，显示 ${count.toLocaleString()} ${noun}。使用方向键导航。`;
	}
	...
	private _showTooltip(index: number | undefined, x: number, y: number): void {
		...
		if (commit.sha !== this._tooltipSha) {
			...
			if (binCount != null && binCount > 1) {
				...
				binRow.textContent = `此 ${this._binUnit} 中还有 ${binCount - 1} 个更多`;
