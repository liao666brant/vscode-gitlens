...
export class GlWipTreePane extends LitElement {
	static override styles = css`
		...
		/* 当面板空间不足时，将储藏标签折叠为仅图标。
		   display:none 会干净地移除已插入的 flex 项，使按钮内部间距也随之折叠——真正的仅图标，
		   没有半截文本。按钮的 tooltip（储藏全部/已暂存更改）在标签隐藏时保持无障碍访问。
		   窄宽度时有意保留组/action-nav 间距，使各操作簇保持视觉区分。 */
	...
	override render() {
		...
		const multiDiffLabel = hasStagedAndUnstaged ? '打开已暂存更改' : '打开所有更改';
		const multiDiffAltLabel = hasStagedAndUnstaged ? '打开未暂存更改' : undefined;
		...
		>
			...
			${files.length > 0
				? html`<div class="wip-actions" slot="leading-actions">
						...
						<gl-action-chip
							icon="gl-stash-save"
							label=${hasStagedAndUnstaged ? '储藏已暂存更改' : '储藏所有更改'}
							alt-label=${hasStagedAndUnstaged ? '储藏所有更改' : nothing}
	...
	private renderDiscardUnstagedAction(files: Files) {
		...
		const label = stagedOnly ? '放弃已暂存更改' : '放弃未暂存更改';
	...
	private renderCopyPatchButton(hasStagedAndUnstaged: boolean) {
		...
		return html`<gl-action-chip
			icon="copy"
			label=${hasStagedAndUnstaged ? '复制已暂存更改（补丁）' : '复制所有更改（补丁）'}
			alt-label=${hasStagedAndUnstaged ? '复制未暂存更改（补丁）' : nothing}
