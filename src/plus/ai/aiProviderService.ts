						...
						if (error instanceof AIError) {
							...
							switch (error.reason) {
								...
								case AIErrorReason.Unreachable: {
									const retry: MessageItem = { title: '重试' };
								...
								case AIErrorReason.UserQuotaExceeded: {
									...
									if (canPurchase) {
										...
										const result = await window.showErrorMessage(
											'您的请求无法完成，因为您已达到当前套餐包含的每周使用量。请购买额外的 AI 额度以继续使用 GitKraken AI。',
											getMoreCredits,
											dismiss,
										);
										...
										await window.showErrorMessage(
											'您的请求无法完成，因为您已达到当前套餐包含的每周使用量。请联系您的组织管理员或所有者以申请更多 AI 额度。',
											ok,
										);
...
function getPickerTitlesForScope(scope: AIModelScope | undefined): {
	...
	if (scope === 'compose') {
		return {
			provider: {
				title: '为撰写选择 AI 提供程序',
				placeholder: '选择用于撰写的 AI 提供程序',
				scope: scope,
			},
			model: {
				title: '为撰写选择 AI 模型',
				placeholder: '选择用于撰写的 AI 模型',
				scope: scope,
			},
		};
	}
	if (scope === 'review') {
		return {
			provider: {
				title: '为评审选择 AI 提供程序',
				placeholder: '选择用于评审的 AI 提供程序',
				scope: scope,
			},
			model: {
				title: '为评审选择 AI 模型',
				placeholder: '选择用于评审的 AI 模型',
				scope: scope,
			},
		};
	}
