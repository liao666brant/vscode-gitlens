...
export async function getBranchNodeParts(
		...
		if (branch.upstream != null) {
			...
			description = options?.showAsCommits
				? `${GitBranch.getTrackingStatus(branch, {
					...
					})}${branch.nameWithoutRemote}${branch.rebasing ? ' (变基中)' : ''}${pad(arrows, 2, 2)}${
			...
			tooltip += `\n\n分支 ${GitBranch.getTrackingStatus(branch, {
	...
	if (branch.date != null) {
		...
		tooltip += `\n\n上次提交于 ${GitBranch.formatDateFromNow(branch)} (${GitBranch.formatDate(
			branch,
	...
	if (options?.showAsCommits) {
		...
		label = `${!options?.useBaseNameOnly ? branchName : branch.basename}${branch.rebasing ? ' (变基中)' : ''}`;
	}
