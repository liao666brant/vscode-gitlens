...
export class ViewCommands implements Disposable {
	...
	private async copyWorkingChangesToWorktree(node: WorktreeNode | UncommittedFilesNode) {
		let worktreeNode: WorktreeNode | undefined;
		if (node.is('uncommitted-files')) {
			const parent = node.getParent()!;
			if (parent?.is('worktree')) {
				worktreeNode = parent as WorktreeNode;
			}
		} else if (node.is('worktree')) {
			worktreeNode = node;
		}
		if (worktreeNode == null) return;

		return WorktreeActions.copyChangesToWorktree(
			'working-tree',
			worktreeNode.worktree.repoPath,
			undefined,
