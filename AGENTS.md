...
**WeGit**（包名 `wegit`，v20.0.0，publisher `liao666brant`）—— 小而美的 Git 工具：在 VS Code 中提供 blame 注释、提交历史可视化、仓库探索与常用 Git 工作流，同时支持桌面版 VS Code（Node.js）与 Web 版 VS Code（browser/webworker）。产品定位与能力概述见文首简介；详细架构见 [docs/architecture.md](docs/architecture.md)。
